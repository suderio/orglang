...
/*
 * Create a string value from a UTF-8 encoded C string.
 * The string data is copied into the arena.
 */
OrgValue org_make_string(Arena *arena, const char *str, size_t byte_len);

/*
 * Get a read-only pointer to the string's UTF-8 data.
 * The string is NOT null-terminated; use org_string_byte_len() for length.
 */
