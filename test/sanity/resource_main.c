...
(org_var_args = right, org_pair_make(arena, org_string_from_c(arena, "args"), org_var_args));
return org_op_infix(arena, "->", org_var_args, org_op_prefix(arena, "@", org_var_stdout));
}
...
int main(int argc, char **argv) {
    ...
    org_op_infix(arena, "->", org_string_from_c(arena, "Start"), org_op_prefix(arena, "@", org_var_stdout));
    org_op_infix(arena, "->", org_op_infix(arena, "->", org_resource_main_create_wrap(arena), org_func_create(arena, org_fn_4)), org_op_prefix(arena, "@", org_var_stdout));
    org_op_infix(arena, "->", org_string_from_c(arena, "End"), org_op_prefix(arena, "@", org_var_stdout));
