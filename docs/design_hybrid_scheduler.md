...
The **OrgLang Hybrid Scheduler** is designed to treat every pulse of data as a task that can be suspended and resumed based on hardware availability. In our model, `@main` is simply the "Seed Pulse" that starts the chain reaction.
...
Every flow starting with `@main` is effectively registered as a "Root Flow" by executing the expression that spawns the initial fibers.

```c
// Pseudo-generated C code from the Go compiler
void org_init_program(OrgContext* ctx) {
    // Flow 1: @main -> "Hello" -> @stdout
    // This executes effectively as:
    // org_sched_spawn_pump(ctx, main_iter, func_hello_stdout);
    
    // Flow 2: @main -> "World" -> @stderr
...
1. **Submission:** The scheduler pulses `@main`. This pulse travels through the graph. When it hits `@stdout`, the resource doesn't "write" and wait. It submits a `SQE` (Submission Queue Entry) to the kernel/scheduler and **yields** its current execution context (the Fiber).
...
**How `@main` works in this context:**
When `@main` pulses, it creates separate Fibers. Each Fiber uses its own **Sub-Arena** (or shares one carefully), allowing for isolation.
